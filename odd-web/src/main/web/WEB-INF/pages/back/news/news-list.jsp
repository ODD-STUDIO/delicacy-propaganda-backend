...
<%--      </div>--%>
  ...
  <xblock class="deleteAll">
    ...
    <button class="layui-btn" onclick="x_admin_show('添加新闻','/back/admin/news/add',800,600)"><i class="layui-icon"></i>添加</button>
  </xblock>
  ...
  layui.use(['table', 'form'], function () {
    ...
    tableIns = table.render({
      ...
      cols: [[
        ...
        { field: 'newsThumb', width: '11%', title: '图片', style: 'height: 50px', templet: function (d) {
            return "<img src='"+'http://img.afblog.love/'+d.url+"' width='200px'/>"
          } },
        { field: 'newsTitle', width: '10%', title: '标题' },
        { field: 'newsDesc', width: '7%', title: '简介' },
        // { field: 'link', width: '10%', title: '图片跳转' },
        { field: 'newsContent', width: '24%', title: '内容'},
        { field: 'newsType.id', title: '类型', width: '9%', templet: function (d) {
            let text;
            let clazz;
            console.log(d.newsType.id)
            console.log(d.newsType.id === 1)
            if (d.newsType.id === 1) {
              text = '行业新闻'
    ...
    table.on('tool(data-table)', function(obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
      ...
      if (layEvent === 'status') { //查看
        ...
        $.ajax({
          type: "post",// 提交的http方法
          url: "/back/admin/news/update", // 提交到后端的接口
        ...
        layer.confirm('真的删除吗?', function (index) {
          ...
          $.ajax({
            ...
            url: "/back/news/delete/" + data.id, // 提交到后端的接口
            dataType: "json", // 后端返回的数据
            success: function (res) { // 后端成功返回数据之后的回调
              if (res.code === 200) {
                // 刷新父级的表格
        ...
        //do something
        // 需要打开编辑页面
        x_admin_show('编辑新闻', '/back/admin/news/update', 800, 600)
    ...
    var $ = layui.$, active = {
      getCheckData: function () { //获取选中数据
        ...
        layer.confirm('真的删除吗?', function (index) {
          $.ajax({
            type: "post",// 提交的http方法
            url: "/back/news/delete/all", // 提交到后端的接口
